...
// ============================================================
// 5. SMART ALERTS
//   Қазірдің өзінде жүктелген technicals/volume/signal негізінде
//   оқиға стиліндегі хабарламалар шығарады. Жаңа API сұраныс
//   ЖОҚ — сондықтан Alpha Vantage лимитіне әсер етпейді.
// ============================================================

/**
 * Қайтарады: [{ text, level }], level: "bullish" | "bearish" | "info"
 */
export function generateSmartAlerts({ technicals, volumeInfo, currentPrice, signal }) {
  const alerts = [];
  if (!technicals) return alerts;

  if (typeof currentPrice === "number" && typeof technicals.ema20 === "number") {
    if (currentPrice > technicals.ema20) {
      alerts.push({ text: "Баға EMA20 деңгейінен жоғары тұр", level: "bullish" });
    } else {
      alerts.push({ text: "Баға EMA20 деңгейінен төмен тұр", level: "bearish" });
    }
  }

  if (
    typeof technicals.ema20 === "number" &&
    typeof technicals.ema50 === "number" &&
    typeof technicals.ema200 === "number"
  ) {
    if (technicals.ema20 > technicals.ema50 && technicals.ema50 > technicals.ema200) {
      alerts.push({ text: "Bullish confluence: EMA20 > EMA50 > EMA200", level: "bullish" });
    } else if (technicals.ema20 < technicals.ema50 && technicals.ema50 < technicals.ema200) {
      alerts.push({ text: "Bearish confluence: EMA20 < EMA50 < EMA200", level: "bearish" });
    }
  }

  if (volumeInfo && typeof volumeInfo.ratio === "number") {
    if (volumeInfo.ratio > 2) {
      alerts.push({ text: `Volume ${volumeInfo.ratio.toFixed(1)}× орташадан жоғары — күшті қызығушылық`, level: "bullish" });
    } else if (volumeInfo.ratio > 1.3) {
      alerts.push({ text: `Volume орташадан ${volumeInfo.ratio.toFixed(1)}× жоғары`, level: "info" });
    } else if (volumeInfo.ratio < 0.5) {
      alerts.push({ text: "Volume орташадан айтарлықтай төмен — қызығушылық аз", level: "info" });
    }
  }

  if (typeof technicals.rsi === "number") {
    if (technicals.rsi < 30) {
      alerts.push({ text: `RSI ${technicals.rsi} — артық сатылған аймақ`, level: "bullish" });
    } else if (technicals.rsi > 70) {
      alerts.push({ text: `RSI ${technicals.rsi} — артық сатып алынған аймақ`, level: "bearish" });
    }
  }

  if (signal && signal.level === "strongBuy") {
    alerts.push({ text: "Мықты СЕНІМДІ САТЫП АЛУ сигналы анықталды", level: "bullish" });
  } else if (signal && signal.level === "strongSell") {
    alerts.push({ text: "Мықты СЕНІМДІ САТУ сигналы анықталды", level: "bearish" });
  }
